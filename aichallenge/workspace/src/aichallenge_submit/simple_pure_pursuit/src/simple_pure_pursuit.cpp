...
namespace simple_pure_pursuit
{
        ...
        steering_tire_angle_gain_(declare_parameter<float>("steering_tire_angle_gain", 1.0))
  {
    ...
    // Use reliable QoS to match typical planners and avoid RELIABILITY mismatch warnings
    const auto sub_qos = rclcpp::QoS(rclcpp::KeepLast(1)).durability_volatile().reliable();
    sub_kinematics_ = create_subscription<Odometry>(
        "input/kinematics", sub_qos, [this](const Odometry::SharedPtr msg)
        { odometry_ = msg; });
    sub_trajectory_ = create_subscription<Trajectory>(
        "input/trajectory", sub_qos, [this](const Trajectory::SharedPtr msg)
        { trajectory_ = msg; });
